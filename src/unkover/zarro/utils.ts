...
import { DimensionSelection, Slice } from "zarr/types/core/types";
import { ChunksArgument, DtypeString, FillType, Order } from "zarr/types/types";
import { isSlice } from "./indexing";
...
function convertColMajorToRowMajorGeneric(
  src: TypedArray,
  out: TypedArray,
  shape: number[]
): void {
 ...
 * @param shape number[]
 */
export function convertColMajorToRowMajor(
  src: TypedArray,
  out: TypedArray,
  shape: number[]
): void {
  return (
    colMajorToRowMajorConverters[shape.length] ||
    convertColMajorToRowMajorGeneric
  )(src, out, shape);
}
...
export function isArrayBufferLike(obj: unknown | null): obj is ArrayBufferLike {
  ...
  if (IS_NODE) {
    // Necessary for Node.js for some reason..
    return (
      (obj as Record<string, unknown>)
        .toString()
        .startsWith("[object ArrayBuffer]") ||
      (obj as Record<string, unknown>)
        .toString()
        .startsWith("[object SharedArrayBuffer]")
    );
  }
  return false;
}
